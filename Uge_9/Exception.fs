...
//floatDiv: float -> float -> float option that divides floating-point numbers and returns None if and only if the second argument is 0.0.

let floatDiv number1 number2 =
    if number2 = 0.0 
    then None
    else Some (number1 / number2) 
...
let floatDiv' x y =
    if y = 0.0 then Error "Division by 0.0"
    else Ok (x / y)

//How does this excepting work ??
exception ZeroDiv of string

let floatDiv'' x y =
    if y = 0.0 then
        raise (ZeroDiv "Division by 0.0")
    else
        x / y

try
    //printfn "%f" (floatDiv'' 10.0 2.0)  // Should print: 5.0
    printfn "%f" (floatDiv'' 10.0 0.0)  // Should raise ZeroDiv exception
with
| ZeroDiv msg -> printfn "Caught exception: %s" msg
